...
//    var imageCollection: [[UIImage]]!
    ...
    override func viewDidLoad() {
        ...
        setupModel()
        addGesture()
        self.userName.text = userStoryListArray[rowIndex].userName
        self.userProfilePic.image = userStoryListArray[rowIndex].userProfile
    }
...
extension StoryViewController {
    ...
    func setupModel() {
        for collection in userStoryListArray {
            arrUser.append(StoryHandler(imgs: collection.statusImage!))
        }
    ...
    func showNextUserStory() {
        ...
        if newUserIndex < arrUser.count {
            ...
            self.userName.text = userStoryListArray[newUserIndex].userName
            self.userProfilePic.image = userStoryListArray[newUserIndex].userProfile
        } else {
            cancelBtnTouched()
        }
    }
    ...
    func showPreviousUserStory() {
        ...
        if newIndex >= 0 {
            ...
            showUpcomingUserStory()
            self.userName.text = userStoryListArray[newIndex].userName
            self.userProfilePic.image = userStoryListArray[newIndex].userProfile
        } else {
            cancelBtnTouched()
        }
